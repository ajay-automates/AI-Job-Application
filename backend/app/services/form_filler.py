...
class FormFillerService:
    ...
    async def _execute_form_filling(
        ...
        # Prepare form data - MUST match form_filler.py expected structure
        form_data = {
            "url": job_url,
            "form_context": {
                "job_url": job_url,
        ...
        # Add resume field - check if resume_url is provided (file path) or resume_text (text)
        if resume_url and os.path.exists(resume_url):
            ...
            # Resume is text (paste into textarea)
            form_data["user_input_template"].append({
                "id": "resume_text",
                "type": "textarea",
